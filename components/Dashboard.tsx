...
import { Star, Lock, PlayCircle, BarChart2 } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ user, onSelectLevel, onChangeLevel }) => {
  const levels: { level: DifficultyLevel; name: string; desc: string }[] = [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid gap-4">
                {levels.map((lvl) => {
                    const isLocked = lvl.level > user.currentLevel + 1; // Can see next level but maybe not skip too far? Let's allow skipping as per prompt.
                    // Prompt says "puede saltar entre niveles". So no locks really, just visual indication of current.
