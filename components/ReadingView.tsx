...
import { evaluateReadingSession } from '../services/genAI';
...
// Simple type for Web Speech API
interface IWindow extends Window {
  webkitSpeechRecognition: any;
  SpeechRecognition: any;
}

export const ReadingView: React.FC<ReadingViewProps> = ({ material, onComplete, onRequestBreak }) => {
  const [isRecording, setIsRecording] = useState(false);
  const [transcript, setTranscript] = useState('');
  const [timeElapsed, setTimeElapsed] = useState(0);
  const [feedback, setFeedback] = useState<{ msg: string; wpm: number } | null>(null);
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => {
                                        ...
                                        if (optIdx === q.correctIndex) {
                                            ...
                                            (e.target as HTMLButtonElement).classList.add('bg-red-200');
                                        }
                                    }}
